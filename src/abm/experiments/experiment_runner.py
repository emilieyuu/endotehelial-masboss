...
class ExperimentRunner:
    ...
    # ------------------------------------------------------------------
    def run_all(self, result_dir=None, suffix=None, save_detail=False, **user_kwargs):
        """
        ...
        if result_dir is not None:
            save_df_to_csv(cell_ts_df, result_dir, "abm_cell_timeseries", suffix=suffix, ts=False)
            save_df_to_csv(cell_ss_df, result_dir, "abm_cell_steady_state", suffix=suffix, ts=False)

            if save_detail: 
                save_df_to_csv(spring_ts_df, result_dir, "abm_spring_timeseries", suffix=suffix, ts=False)
                save_df_to_csv(node_ts_df, result_dir, "abm_node_timeseries", suffix=suffix, ts=False)
                save_df_to_csv(spring_ss_df, result_dir, "abm_spring_steady_state", suffix=suffix, ts=False)
                save_df_to_csv(node_ss_df, result_dir, "abm_node_steady_state",suffix=suffix, ts=False)
